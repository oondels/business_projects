<template>
  ...
  <div class="container-fluid">
    ...
    <gradient-line-chart
      v-if="mostrarGrafico"
      :customClass="'rounded bg-gradient-dark text-white'"
      :title="'Totais: Gasto(R$) X Consumo(Kg)'"
      :titleStyle="'text-white p-0'"
      :description="descricaoPerformance"
      id="grafico"
      :chart="graficoTotalConsumoGasto"
    />
    ...
    <div class="col-12 p-0 m-0 mb-2 row">
      <div class="col-lg-6 col-sm-12">
        <gradient-line-chart
          ...
          :titleStyle="'text-white p-0'"
          :description="descricaoPerformance"
          id="graficoTotalProdutivoResiduoKG"
          :chart="graficoTotalProdutivoResiduoKG"
        />
      </div>
      ...
      <div class="col-lg-6 col-sm-12'">
        <gradient-line-chart
          ...
          :titleStyle="'text-white p-0'"
          :description="descricaoPerformance"
          id="graficoTotalProdutivoResiduoRS"
          :chart="graficoTotalProdutivoResiduoRS"
        />
      </div>
    </div>
    ...
    <div class="col-12 row mb-2">
      ...
      <div class="col-9 row">
        <v-select
          ...
          variant="outlined"
          label="Tipo"
          :items="['', 'Pacotes', 'Individuais']"
          v-model="tipo"
          @update:modelValue="buscaTodasSolicitacoes()"
        ></v-select>
        <v-select
          class="text-black col-2"
          density="compact"
    ...
    <ul class="lis-group p-0 rounded lista" v-if="mostraSolicitacoes">
      ...
      <li class="list-group-item" v-for="solicitacao in solicitacoes" :key="solicitacao.id">
        <div class="col-12 row">
          ...
          <div class="col-2">
            <span>
              {{ solicitacao.nome_solicitante.split(" ")[0] + " " + solicitacao.nome_solicitante.split(" ").pop() }}
            </span>
          </div>
          <div class="col-2">
            <span>
              {{ solicitacao.gerente.split(" ")[0] + " " + solicitacao.gerente.split(" ").pop() }}
            </span>
          </div>
...
export default {
  ...
  methods: {
    ...
    buscarDadosProdutos(categoria, individual, produto) {
      ...
      axios
        .get(`http://${ip}:3045/buscaDadosProdutos`, {
          params: { categoria: categoria, individual: individual, produto: produto },
        })
        .then((response) => {
