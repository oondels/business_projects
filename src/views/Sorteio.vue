    ...
    <div class="vh-100 d-flex justify-content-center align-items-center" :style="{
        ...
        <div class="stepper rounded px-2 py-3 vw-100 shadow-none transform-scale-md-9">
            ...
            <div class="col-12 row p-0 m-0 mb-2">
                ...
                <v-text-field type="number" class="col-2" label="Quantidade de ganhadores"
                    v-model="sorteio.quantidadeGanhadores"></v-text-field>
            </div>
            <div class="col-12 row mb-4">
                ...
                <v-dialog max-width="full-screen" v-model="modalContador" class="bg-white">
                    ...
                    <v-card class="col-12 text-center" v-if="terminouContagem">
                        ...
                        <v-card-actions>
                            <v-spacer></v-spacer>
                            <v-btn class="ms-auto" color="danger" text="Fechar"
                                @click="modalContador = false; ganhadoresImediatos = []"></v-btn>
                        </v-card-actions>
                    </v-card>
                </v-dialog>
            </div>
            <div class="col-12">
                <div class="col-12 row mb-2">
                    ...
                    <div class="col-2">
                        <v-select density="compact" variant="outlined" :items="filtrar.gerentes" label="Gerente"
                            v-model="filtro.gerente" @update:modelValue="this.buscaGanhadores()"></v-select>
                    </div>
                    <div class="col-2">
                        <v-select density="compact" variant="outlined" :items="filtrar.setores" label="Setor"
                            v-model="filtro.setor" @update:modelValue="this.buscaGanhadores()"></v-select>
                    </div>
                    <div class="col-2">
                        <v-select density="compact" variant="outlined" :items="['FALTA ZERO', 'PENSE & AJA']"
                            label="Evento" v-model="filtro.evento"
                            @update:modelValue="this.buscaGanhadores()"></v-select>
                    </div>
                    <div class="col-2">
                        <v-text-field density="compact" variant="outlined" type="date" label="Data do sorteio"
                            v-model="filtro.dataSorteio" @update:modelValue="this.buscaGanhadores()"></v-text-field>
                    </div>
                </div>
                <ul class="list-group">
...
export default {
    ...
    methods: {
        ...
        buscaGerentes() {
            ...
            axios
                .get(`http://${ip}:3050/buscaGerentes`)
                .then((response) => {
        ...
        buscaGanhadores() {
            axios
                .get(`http://${ip}:3050/ganhadores`, { params: { filtro: this.filtro } })
                .then((response) => {
