            ...
            <div class="col-lg-12 position-relative">
                <div v-if="permissaoRefeitorio()" class="card p-0 mb-4 px-2">
                    ...
                    <div class="row d-flex justify-content-between col-12">
                        <v-switch color="primary" class="col-8" v-model="reservarSabadoNaSexta"
                            :label="`Reservar sábado na sexta: ${reservarSabadoNaSexta === true ? 'Sim' : 'Não'}`"
                            hide-details inset @change="salvaConfiguracao()"></v-switch>
                        <div class="col-4 row">
                            <v-text-field class="col-6 text-center" type="date" label="Data para reserva diária"
                                v-model="dataAmanhaParaReserva" @change="salvaConfiguracao()"></v-text-field>

                            <v-text-field class="col-6 text-center" type="date" label="Data para reserva extra"
                                v-model="dataReserva" @change="salvaConfiguracao()"></v-text-field>
                        </div>
                    </div>
                </div>
                ...
                <div class="row position-sticky fixed-top">
                    <div class="col-xl-4 col-sm-12 mb-xl-0">
                        <mini-statistics-card2 title="Reservas totais" :value="reservasAmanhaDoDia.length"
                            :consumido="reservasAmanhaConsumidas.length" :icon="{
                                component: 'confirmation_number',
                                background: 'bg-gradient-dark',
                            }" direction-reverse />
                    </div>
                    <div class="col-xl-4 col-sm-6 mb-xl-0">
                        <mini-statistics-card2 title="Light" :value="reservasAmanhaLight.length"
                            :consumido="reservasAmanhaLightConsumidas.length" :icon="{
                                component: 'dinner_dining',
                                background: 'bg-gradient-dark',
                            }" direction-reverse />
                    </div>
                    <div class="col-xl-4 col-sm-6 mb-xl-0">
                        <mini-statistics-card2 title="Lanche" :value="reservasAmanhaLanche.length"
                            :consumido="reservasAmanhaLancheConsumidas.length" :icon="{
                ...
                <div class="col-lg-12 col-sm-12">
                    <div class="card col-12 z-index-2 mb-4">
                        <gradient-line-chart v-if="showChart" id="performance1" title="Refeitório"
                            :description="'Reservado x Consumido'" :chart="chartData" />
                    </div>
                </div>
                <h5 class="mb-2">Ações</h5>
                <div class="row">
                    <router-link v-if="permissaoRefeitorio()" to="/refeitorio/reserva"
                        class="col-lg-3 col-md-6 col-sm-6 mt-lg-0 mt-4">
                        <mini-statistics-card :title="{ text: 'Reservar' }" :icon="{
                            name: 'lunch_dining',
                            color: 'text-white',
                            background: 'primary',
                        }" />
                    ...
                    <v-dialog full>
                        <template v-if="permissaoRefeitorio()" v-slot:activator="{ props: activatorProps }">
                            <div v-bind="activatorProps" class="col-lg-3 col-md-6 col-sm-6 mt-lg-0 mt-4 cursor-pointer">
                                <mini-statistics-card :title="{ text: 'Reservados' }" :icon="{
                        ...
                        <template v-slot:default="{ isActive }">
                            <v-card class="z-4">
                                ...
                                <consumo-refeicao :reservasDoDia="reservasDoDia" :reservasLanche="reservasLanche"
                                    :reservasLight="reservasLight" :reservasLancheConsumidas="reservasLancheConsumidas"
                                ...
                                <v-card-actions>
                                    <v-spacer></v-spacer>
                                    <v-btn color="danger" text="Fechar" variant="text"
                    ...
                    <v-dialog max-width="800">
                        <template v-if="autorizaReservarSexta()" v-slot:activator="{ props: activatorProps }">
                            <div v-bind="activatorProps" class="col-lg-3 col-md-6 col-sm-6 mt-lg-0 mt-4 cursor-pointer">
                                <mini-statistics-card :title="{ text: 'Sábado/Feriado' }" :icon="{
                                    name: 'restaurant',
                                    color: 'text-white',
                                    background: 'success',
                                }" />
                                ...
                                <v-card-title
                                    ...
                                    <div class="col-12 insercao-informacao">
                                        <div class="col-12 row">
                                            <v-select class="col-6" v-model="gerenteSelecionado"
                                                @update:modelValue="buscaSetores(gerenteSelecionado)"
                                                label="Gerente do colaborador" :items="gerentes" autofocus></v-select>
                                            <v-select class="col-6" :disabled="disabledSetores"
                                                label="Setor do colaborador" :items="setores" v-model="setorSelecionado"
                                                @update:modelValue="buscaColaboradoresPorSetor"></v-select>
                                        </div>

                                        <div class="col-12 row">
                                            <v-select :disabled="disabledCelula" class="col-6" :items="gerentes"
                                                label="Gerente da hora extra" v-model="gerenteDestino"
                                                @update:modelValue="buscaSetores(gerenteDestino)"></v-select>
                                            <v-select class="col-6" :disabled="disabledCelula"
                                                label="Setor da hora extra" :items="setores"
                                                v-model="setorDestino"></v-select>
                                        </div>

                                        <div class="col-12 row">
                                            <v-text-field disabled class="col-6" type="date" label="Data da Reserva"
                                                v-model="dataReserva"></v-text-field>
                                            <v-text-field :disabled="disabledCelula" class="col-6" type="time"
                                                label="Hora da reserva" v-model="horaReserva"></v-text-field>
                                        </div>
                                    </div>
                                </v-card-title>
                                <v-card-item>
                                    <listagem-colaborador v-if="!disabledCelula" ref="listagemColaborador"
                                        :colaboradoresSabado="colaboradoresSabado" :dataReserva="dataReserva"
                                        :horaReserva="horaReserva" :gerenteSelecionado="gerenteSelecionado"
                                        :gerenteDestino="gerenteDestino" :setorSelecionado="setorSelecionado"
                                        :setorDestino="setorDestino" @atualizar-informacao="atualizarInformacao" />
                                </v-card-item>

                                <v-card-actions class="position-fixed fixed-bottom bg-white border-top">
                                    <v-spacer></v-spacer>
                                    <v-btn color="danger" text="Fechar" variant="flat"
                                        @click="isActive.value = false"></v-btn>
                                    <v-btn color="success" text="Salvar" @click="emitirSalvarColaboradores"
                                        variant="flat"></v-btn>
                                </v-card-actions>
                            </v-card>
                        </template>
                    ...
                    <v-dialog max-width="800">
                        <template v-if="permissaoRefeitorio()" v-slot:activator="{ props: activatorProps }">
                            <div @click="buscaGerentes" v-bind="activatorProps"
                                ...
                                <mini-statistics-card :title="{ text: 'Relatórios' }" :icon="{
                                    name: 'receipt_long',
                                    color: 'text-white',
                                    background: 'info',
                                }" />
                                ...
                                <v-card-title>
                                    ...
                                    <div class="col-12 row">
                                        ...
                                        <div class="col-6 text-center">
                                            <v-btn @click="triagemRelatorio('QuantidadeSabado')">Quantidade para o
                                                sábado</v-btn>
                                        </div>
                                        <div class="col-6 text-center">
                                ...
                                <v-card-item>
                                    <div v-if="gerarRelatorio">
                                        ...
                                        <div class="col-12 row">
                                            <div class="col-4">
                                                <v-text-field type="date" label="Data inicial"
                                                    v-model="dataInicial"></v-text-field>
                                            </div>
                                            <div class="col-4">
                                                <v-text-field type="date" label="Data final"
                                                    v-model="dataFinal"></v-text-field>
                                            </div>
                                            <div class="col-4">
                                                <v-select :items="['Todos', 'Turno A', 'Turno B']"
                                                    label="Turno de reserva" v-model="turnoSelecionado"></v-select>
                                            </div>
                                        </div>
                                        ...
                                        <v-card-actions class=" bg-white border-top">
                                            ...
                                            <v-btn color="danger" text="Fechar" variant="flat"
                                                @click="isActive.value = false"></v-btn>
                                            <v-btn color="success" text="Baixar"
                                    ...
                                    <div v-if="gerarQuantidadeSabado" class="col-12 insercao-informacao">
                                        ...
                                        <v-text-field disabled class="col-4" type="date" label="Data"
                                            v-model="dataReserva"></v-text-field>
                                    ...
                                    <div v-if="gerarVoucherSabado" class="col-12 insercao-informacao">
                                        ...
                                        <v-select class="col-6" :items="gerenteReservado" label="Gerentes reservados"
                                            v-model="gerenteReservadoSelecionado"></v-select>
...
export default {
    ...
    methods: {
        ...
        permissaoRefeitorio() {
            ...
            if (this.decodeJwt().setor === "AUTOMACAO" || this.decodeJwt().setor === "PORTARIA" || this.decodeJwt().setor === "REFEITORIO" || this.decodeJwt().setor === "ADMINISTRATIVO") {
                return true;
            }
        },
        ...
        buscaConfiguracao() {
            axios
                ...
                .then((response) => {
                    this.dataAmanhaParaReserva = this.pegaProximoDiaUtil() > response.data.dataamanha.split("T")[0] ? this.pegaProximoDiaUtil() : response.data.dataamanha.split("T")[0]
        ...
        buscaColaboradoresPorSetor() {
            if (this.gerenteSelecionado && this.setores && this.dataReserva) {
                axios
                    .get(`http://${ip}:3048/buscaColaboradoresPorSetor`, { params: { gerente: this.gerenteSelecionado, setor: this.setorSelecionado, dataReserva: this.dataReserva } })
                    .then(response => {
        ...
        addCard(doc, pessoa, xPos, yPos, width, height) {
            ...
            doc.text(`Gerente: ${pessoa.gerente.split(" ")[0] + " " + pessoa.gerente.split(" ").pop()}`, xPos + 1, yPos + 2 + lineHeight);
            ...
            doc.text(`Gerente de Destino: ${pessoa.gerenteDestino.split(" ")[0] + " " + pessoa.gerenteDestino.split(" ").pop()}`, xPos + 1, yPos + 1 + lineHeight * 6);
        ...
        geraRelatorioLancheSabado(tipoRelatorio) {
            axios
                .get(`http://${ip}:3048/geraRelatorioLancheSabado`, { params: { tipoRelatorio: tipoRelatorio, dataInicial: this.dataInicial, dataFinal: this.dataFinal, turno: this.turnoSelecionado } })
                .then((response) => {
