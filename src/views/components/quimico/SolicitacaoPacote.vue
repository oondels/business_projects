    ...
    <v-dialog max-width="800">
        <template v-slot:activator="{ props: activatorProps }">
            <div v-bind="activatorProps" class="col-lg-4 col-md-6 col-sm-6 mt-lg-0 mt-4 cursor-pointer">
                <mini-statistics-card :title="{ text: 'Solicitação de pacote' }" :icon="{
                    name: 'dataset',
                    color: 'text-white',
                    background: 'primary',
                }" />
            ...
            <v-card>
                ...
                <v-card-item>
                    <div class="col-12 row">
                        <div class="col-6">
                            <v-select :items="processos" label="Processo" v-model="solicitacao.processo"
                                @update:modelValue="buscaModelosCadastrados(solicitacao.processo)"></v-select>
                        </div>
                        <div class="col-6">
                            <v-select :disabled="!solicitacao.processo" :items="dadosModelo" return-object
                                item-title="modelo" item-value="id" label="Modelo" v-model="dadosModeloSelecionado"
                                @update:modelValue="atribuiModeloSolicitacao(dadosModeloSelecionado)"></v-select>
                        </div>
                        <div class="col-6">
                            <v-text-field type="number" label="Produção prevista"
                                v-model="solicitacao.producao"></v-text-field>
                        </div>
                        <div class="col-6">
                            <v-text-field type="number" label="Célula" v-model="solicitacao.celula"></v-text-field>
                        </div>
                        <div class="col-6">
                            <v-select :items="['Fábrica 1', 'Fábrica 2', 'Fábrica 3']" label="Fábrica"
                                v-model="solicitacao.fabrica"></v-select>
                        </div>
                        <div class="col-6">
