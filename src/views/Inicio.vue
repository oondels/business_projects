<template>
  <div class="py-4 container-fluid">
      ...
      <div class="col-lg-12 position-relative z-index-2">
        <div class="row">
          <div class="col-lg-4 col-md-6 col-sm-6">
            <mini-statistics-card
              v-if="producao"
              :title="{ text: 'Produção de ontem', value: `${(producao.media * 100).toFixed(2)}%` }"
              :icon="{
                name: 'leaderboard',
                color: 'text-white',
                background: 'dark',
          ...
          <div class="col-lg-4 col-md-6 col-sm-6 mt-lg-0">
            <mini-statistics-card
              v-if="qualidade"
              :title="{ text: 'Qualidade', value: `${(qualidade.media * 100).toFixed(2)}%` }"
              :icon="{
                name: 'leaderboard',
                color: 'text-white',
                background: 'info',
          ...
          <div class="col-lg-4 col-md-12 col-sm-6 mt-lg-0">
            <mini-statistics-card
              v-if="eficiencia"
              :title="{ text: 'Eficiência média mensal', value: `${(eficiencia.media * 100).toFixed(2)}%` }"
              :icon="{
                name: 'leaderboard',
                color: 'text-white',
                background: 'success',
        ...
        <div class="row mt-4">
          <div class="col-lg-4 col-md-6 col-sm-6 mt-lg-0 mt-4 mb-4">
            <chart-holder-card
              title="Produção real"
              subtitle="Montante produzido na fábrica"
              update="Atualizado diariamente"
              color="dark"
            >
              <reports-line-chart
                id="tasks-chart"
                ...
                :chart="{
                  ...
                  datasets: {
                    label: 'Produção',
                    data: [
                      (producao.seg * 100).toFixed(2),
                      (producao.ter * 100).toFixed(2),
                      (producao.qua * 100).toFixed(2),
                      (producao.qui * 100).toFixed(2),
                      (producao.sex * 100).toFixed(2),
                    ],
                  },
                }"
              />
          ...
          <div class="col-lg-4 col-md-6 col-sm-6 mt-lg-0 mt-4">
            <chart-holder-card
              title="Qualidade (DI)"
              subtitle="Geral de DI da fábrica"
              update="Atualizado diariamente"
              color="info"
            >
              <reports-line-chart
                ...
                :chart="{
                  ...
                  datasets: {
                    label: 'Qualidade',
                    data: [
                      (qualidade.seg * 100).toFixed(2),
                      (qualidade.ter * 100).toFixed(2),
                      (qualidade.qua * 100).toFixed(2),
                      (qualidade.qui * 100).toFixed(2),
                      (qualidade.sex * 100).toFixed(2),
                    ],
                  },
                }"
              />
              ...
              <reports-line-chart
                ...
                :chart="{
                  ...
                  datasets: {
                    ...
                    data: [
                      (eficiencia.seg * 100).toFixed(2),
                      (eficiencia.ter * 100).toFixed(2),
                      (eficiencia.qua * 100).toFixed(2),
                      (eficiencia.qui * 100).toFixed(2),
                      (eficiencia.sex * 100).toFixed(2),
                    ],
                  },
                }"
              />
    ...
    <v-carousel show-arrow="hover" cycle hide-delimiter-background class="col-lg-12 col-md-12 mb-4">
      <v-carousel-item v-for="item in showDestaques" :key="item.id" :src="openImagem(item.caminho)" cover></v-carousel-item>
    </v-carousel>
    ...
    <div class="row">
      <div class="col-lg-8 col-md-6 mb-md-0 mb-4">
        <distack-list
          title="Pense & Ajas"
          description="Destaques pense & ajas classse A"
          :headers="['Projeto', 'Turno', 'Realizado', 'Executor']"
          :projects="penseAjas"
        />
      </div>
      <div class="col-lg-4 col-md-6">
        ...
        <timeline-list class="h-100" title="Palavras gerenciais">
          <timeline-item
            v-for="item in showPalavraGerencial"
            :key="item.id"
            :icon="{
              component: 'article',
              class: 'text-primary',
...
export default {
  ...
  methods: {
    ...
    adicionarDestaque() {
      ...
      axios
        .post(`http://${ip}:3041/adicionarDestaque`, formData, {
          headers: {
    ...
    buscaDestaques() {
      axios
        .get(`http://${ip}:3041/buscaDestaques`)
        .then((response) => {
    ...
    adicionarPalavraGerencial() {
      ...
      axios
        .post(`http://${ip}:3041/adicionarPalavraGerencial`, formData, {
          headers: {
            "Content-Type": "multipart/form-data",
          },
        })
    ...
    buscaPalavrasGerenciais() {
      axios
        .get(`http://${ip}:3041/getPalavrasGerenciais`)
        .then((response) => {
    ...
    openPdf(caminho) {
      window.open(`http://${ip}:3041/${caminho}`, "_blank");
    },

    buscaPenseAja() {
      axios
        .get(`http://${ip}:3041/buscaPenseAja`)
        .then((response) => {
