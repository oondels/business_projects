...
export default {
  ...
  methods: {
    getCandidates() {
      axios
        .get(`http://${ip}:3051/get-candidates`)
        .then((response) => {
    ...
    postVote(employeeVote, choice) {
      ...
      axios
        .post(`http://${ip}:3051/post-vote`, vote)
        .then(() => {
    ...
    validadeVote(data, choice) {
      ...
      axios
        .get(`http://${ip}:3051/validate-vote`, { params: { userRfid: data } })
        .then((response) => {
