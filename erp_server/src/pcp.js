...
app.post("/salvaEntrega", async (req, res) => {
  try {
    const { rfid, nome, matricula, pedido, produto, descricao, usuario } =
      req.body;
    ...
    const salvaEntrega = await pool.query(
      "INSERT INTO pcp.entrega_pedido (createdate, rfid, nome, matricula, pedido, produto, descricao, usuariocreate) VALUES('NOW()', $1, $2, $3, $4, $5, $6, $7)",
      [rfid, nome, matricula, pedido, produto, descricao, usuario]
    );
...
app.get("/buscaEntregas", async (req, res) => {
  try {
    ...
    const entregas = await pool.query(
      "SELECT * FROM pcp.entrega_pedido WHERE DATE(createdate) = $1",
      [data]
    );
...
app.get("/buscaColaboradorRfid", async (req, res) => {
  try {
    ...
    const colaborador = await pool.query(
      "SELECT * FROM colaborador.funcionario WHERE codbarras = $1 LIMIT 1",
      [codigoRfid]
    );
...
app.get("/buscaColaboradorMatricula", async (req, res) => {
  try {
    ...
    const colaborador = await pool.query(
      "SELECT * FROM colaborador.lista_funcionario WHERE matricula = $1 LIMIT 1",
      [matricula]
    );
    res.status(200).json(colaborador.rows[0]);
  } catch (error) {
...
app.get("/buscaPedido", async (req, res) => {
  try {
    ...
    const dadosPedido = await pool.query(
      "SELECT * FROM pcp.pedidos WHERE docto = $1 LIMIT 1",
      [pedido]
    );
