...
app.get('/colaboradores', async (req, res) => {
    try {
        ...
        const colaboradores = await pool.query(`SELECT nome, gerente, nome_setor FROM colaborador.lista_funcionario WHERE matricula = $1 LIMIT 1`, [matricula]);
...
app.post('/produtos', async (req, res) => {
    try {
        ...
        if (!codigo || !ordem || !colaborador.matricula || !colaborador.nome || !colaborador.gerente || !colaborador.setor) {
            return res.status(422).send("Dados inválidos ou ausentes");
        }

        const verificaDisponibilidade = await pool.query("SELECT * FROM produtos.produtos_cadastrados WHERE codigo = $1 OR ordem = $2", [codigo, ordem]);
        if (verificaDisponibilidade.rowCount > 0) {
            return res.status(409).send("Código ou ordem já cadastrados");
        }

        const produtos = await pool.query(`INSERT INTO produtos.produtos_cadastrados (createdate, codigo, ordem, matricula, nome, gerente, setor) VALUES ('NOW()', $1, $2, $3, $4, $5, $6)`, [codigo, ordem, colaborador.matricula, colaborador.nome, colaborador.gerente, colaborador.setor]);
...
app.get('/produtos', async (req, res) => {
    try {
        ...
        if (filtro && filtro.data) {
            ...
            queryEntradaExpedicao += `AND pe.createdate::date = $${params.length + 1} `;
        ...
        return res.status(200).json({ saidaMontagem: saidaMontagem.rows, entradaExpedicao: entradaExpedicao.rows, saidaExpedicao: saidaExpedicao.rows });
    } catch (error) {
        return res.status(500).send("Erro interno do servidor")
    }
})
