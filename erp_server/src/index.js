...
import { tokenValidated, PRIVATE_KEY } from "./auth.js";
...
app.post('/login', async (req, res) => {
    try {
        ...
        const result = await pool.query('SELECT * FROM autenticacao.usuarios WHERE usuario = $1 AND senha = $2', [usuario.toUpperCase(), parseMD5(senha)]);

        if (result.rows.length === 0) return res.status(401).send('Usuário ou senha incorreta!');
        const dadosBanco = result.rows[0];

        const token = jsonwebtoken.sign(
            { usuario: dadosBanco.usuario, codbarras: dadosBanco.codigo_barras, rfid: dadosBanco.rfid, matricula: dadosBanco.matricula, setor: dadosBanco.setor, nivel: dadosBanco.nivel, unidade: dadosBanco.unidade },
            PRIVATE_KEY
        );
...
app.get('/buscaPenseAja', async (req, res) => {
    try {
        const result = await pool.query("SELECT situacao_atual, nome_projeto AS title, turno, TO_CHAR(data_realizada, 'DD/MM') AS data, (regexp_split_to_array(nome, ' '))[1] || ' ' || (regexp_split_to_array(nome, ' '))[array_length(regexp_split_to_array(nome, ' '), 1)] AS nome FROM pense_aja.pense_aja WHERE status_analista = 'APROVAR' AND classificacao = 'A' ORDER BY id DESC LIMIT 6")
...
app.get('/buscaDestaques', async (req, res) => {
    try {
        const result = await pool.query("SELECT id, createdate, arquivo, caminho, titulo FROM unix.arquivos WHERE arquivo = 'DESTAQUE' AND createdate >= CURRENT_DATE - INTERVAL '2 months' ORDER BY id DESC");
...
app.get('/getPalavrasGerenciais', async (req, res) => {
    try {
        const result = await pool.query("SELECT id, createdate, arquivo, caminho, titulo FROM unix.arquivos WHERE arquivo = 'PALAVRA GERENCIAL' AND createdate >= CURRENT_DATE - INTERVAL '2 months' ORDER BY id DESC");
        const items = result.rows.map(row => ({
            id: row.id,
            titulo: row.titulo,
            dateTime: new Date(row.createdate).toLocaleDateString('pt-BR'),
...
app.get("/buscaColaboradorPelaMatricula", async (req, res) => {
    try {
        ...
        const result = await pool.query("SELECT nome, matricula, gerente, nome_setor FROM colaborador.lista_funcionario WHERE matricula = $1", [matricula]);
...
app.post("/salvaPesquisa", async (req, res) => {
    try {
        ...
        if (!pesquisa.turno || !pesquisa.diaGozado || !pesquisa.diaCompensacao || !pesquisa.matricula || !pesquisa.nome || !pesquisa.gerente || !pesquisa.setor) {
        ...
        const salvaPesquisa = await pool.query("INSERT INTO unix.pesquisa_junina(createdate, turno, dia_gozado, dia_compensacao, matricula, nome, gerente, setor) VALUES('NOW()', $1, $2, $3, $4, $5, $6, $7)",
