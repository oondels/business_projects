...
app.post("/cadastrarModelo", async (req, res) => {
    ...
    try {
        ...
        const verificaModelo = await client.query(`SELECT modelo FROM quimico.modelo WHERE modelo = $1 AND processo = $2`, [modelo.modelo.toUpperCase(), modelo.processo.toUpperCase()])
        ...
        if (salvaModelo.rowCount > 0) {
            ...
            for (const produto of produtos) {
                ...
                await client.query(`
                    ...
                    [produto.produto.toUpperCase(), produto.consumoPrevio, produto.precoKg, produto.base.toUpperCase(), produto.recipientes, usuario.toUpperCase(), dadosModelo.id, dadosModelo.modelo.toUpperCase(), modelo.processo.toUpperCase()]);
            }
...
app.get('/buscaModelosCadastrados', async (req, res) => {
    try {
        ...
        const modelosCadastrados = await pool.query(`SELECT id, modelo, marca FROM quimico.modelo WHERE processo = $1 ORDER BY modelo;`, [processo])
...
app.post('/salvarSolicitacaoPacote', async (req, res) => {
    try {
        ...
        if (!solicitacao.modelo || !solicitacao.idModelo || !solicitacao.producao || !solicitacao.celula || !solicitacao.turno || !solicitacao.fabrica || !solicitacao.nome || !solicitacao.matricula || !solicitacao.gerente || !solicitacao.processo || !solicitacao.marca) {
            return res.status(422).send("Dados inválidos ou ausentes")
        }

        const verificaModelo = await pool.query(`SELECT modelo FROM quimico.solicitacoes_pacotes WHERE id_modelo = $1 AND entregue = false AND createdate::date = CURRENT_DATE AND celula = $2;`, [solicitacao.idModelo, solicitacao.celula])
        ...
        const salvaSolicitacaoPacote = await pool.query(`
            ...
            [solicitacao.modelo, solicitacao.idModelo, solicitacao.producao, solicitacao.celula, solicitacao.turno, solicitacao.fabrica, solicitacao.nome, solicitacao.matricula, solicitacao.gerente, solicitacao.processo, solicitacao.marca]
        )
...
app.get('/buscaProdutosCadastrados', async (req, res) => {
    try {
        ...
        const produtosCadastrados = await pool.query(`SELECT DISTINCT MAX(id) AS id, produto, MAX(preco_kg) as preco_kg FROM quimico.produtos GROUP BY produto ORDER BY produto;`)
...
app.post('/salvarSolicitacaoIndividual', async (req, res) => {
    try {
        ...
        if (!solicitacao.produto || !solicitacao.idProduto || !solicitacao.celula || !solicitacao.turno || !solicitacao.nome || !solicitacao.matricula || !solicitacao.gerente || !solicitacao.recipientes || !solicitacao.preco_kg || !solicitacao.marca || !solicitacao.motivo) {
            return res.status(422).send("Dados inválidos ou ausentes");
        }
...
app.post('/manipulaAbastecimento', async (req, res) => {
    try {
        ...
        if (abastecimento.instrucao === 'inicio') {
            const inicio = await pool.query(`UPDATE quimico.solicitacoes_pacotes SET abastecendo = true, data_inicio = NOW(), usuario_inicio = $1 WHERE id = $2`, [abastecimento.usuario, abastecimento.id])
            return res.status(200).send("Iniciado com sucesso")
        }
        if (abastecimento.instrucao === 'fim') {
            const fim = await pool.query(`UPDATE quimico.solicitacoes_pacotes SET entregue = true, data_fim = NOW(), usuario_fim = $1, abastecimento = $2 WHERE id = $3`, [abastecimento.usuario, abastecimento.abastecimentos, abastecimento.id]);
...
app.put('/cancelarSolicitacao', async (req, res) => {
    try {
        ...
        const verificaDisponibilidade = await pool.query(`SELECT * FROM quimico.solicitacoes_pacotes WHERE id = $1 AND matricula_solicitante = $2`, [id, matricula]);
        if (verificaDisponibilidade.rowCount === 0) {
            return res.status(404).send("Esta solicitação não foi feita pela matrícula fornecida");
        }

        const cacelaSolicitacao = await pool.query(`UPDATE quimico.solicitacoes_pacotes SET cancelado = true, motivo_cancelamento = $1 WHERE id = $2`, [motivoSelecionado, id]);
...
app.post('/salvaAbastecimentoIndividual', async (req, res) => {
    try {
        ...
        if (abastecimento.instrucao === 'inicio') {
            await pool.query(`UPDATE quimico.solicitacoes_individuais SET abastecendo = true, data_inicio = NOW(), usuario_inicio = $1 WHERE id = $2`, [abastecimento.usuario, abastecimento.id]);
            return res.status(200).send("Iniciado com sucesso");
        } else if (abastecimento.instrucao === 'fim') {
            await pool.query(`UPDATE quimico.solicitacoes_individuais SET entregue = true, data_fim = NOW(), usuario_fim = $1, residuo = $2, abastecido = $3, mes = $4 WHERE id = $5`, [abastecimento.usuario, abastecimento.residuo, abastecimento.abastecido, abastecimento.mes, abastecimento.id]);
            return res.status(200).send("Finalizado com sucesso");
        } else {
...
app.get('/buscaDadosProdutos', async (req, res) => {
    try {
        ...
        const graficoTotalProdutivoResiduoKG = montaGrafico(buscaDados.rows, 'Consumo: Produtivo x Resíduo', 'total_produtivo', 'total_residuo', 'Produtivo(Kg)', 'Resíduo(Kg)');
        const graficoTotalProdutivoResiduoRS = montaGrafico(buscaDados.rows, 'Custo: Produtivo x Resíduo', 'total_gasto_produtivo', 'total_gasto_residuo', 'Produtivo(R$)', 'Resíduo(R$)');
        const graficoTotalConsumoGasto = montaGrafico(buscaDados.rows, 'Total Consumo x Gasto', 'total_gasto', 'total_consumido', 'Gasto(R$)', 'Consumido(Kg)');
...
app.post('/manipularProduto', async (req, res) => {
    try {
        ...
        if (!produto.produto || !produto.modelo || !produto.processo || !produto.recipientes || !produto.consumo_previo || !produto.preco_kg || !produto.usuariocreate || !produto.id_modelo || !produto.base) {
            return res.status(400).send("Dados de produto não fornecidos");
        }
        ...
        const salvaProduto = await pool.query(`
            INSERT INTO quimico.produtos (produto, consumo_previo, preco_kg, base, recipientes, createdate, usuariocreate, id_modelo, modelo, processo)
            VALUES ($1, $2, $3, $4, $5, NOW(), $6, $7, $8, $9);`,
            [produto.produto.toUpperCase(), produto.consumo_previo, produto.preco_kg, produto.base.toUpperCase(), produto.recipientes, produto.usuariocreate.toUpperCase(), produto.id_modelo, produto.modelo.toUpperCase(), produto.processo.toUpperCase()]);
...
app.post('/cracha', async (req, res) => {
    try {
        ...
        const salvaAltorizacao = await pool.query(`INSERT INTO quimico.autorizados (createdate, matricula, nome, gerente, setor, autorizante) VALUES ('NOW()', $1, $2, $3, $4, $5)`,
...
app.get('/solicitacoes', async (req, res) => {
    try {
        ...
        const buscaGerentes = await pool.query(`SELECT DISTINCT gerente FROM quimico.solicitacoes_${consulta} WHERE entregue = true ORDER BY gerente ASC;`);
