...
app.get("/buscaColaboradorPeloRfid", async (req, res) => {
    try {
        ...
        const result = await pool.query("SELECT nome, matricula FROM colaborador.colaboradores WHERE rfid = $1", [rfid]);
...
app.get("/buscaColaboradorPelaMatricula", async (req, res) => {
    try {
        ...
        const result = await pool.query("SELECT nome FROM colaborador.lista_funcionario WHERE matricula = $1", [matricula]);
...
app.post("/salvaConfiguracao", async (req, res) => {
    try {
        const { reservarSabadoNaSexta, dataAmanhaParaReserva, dataReservaExtra, usuarioCreate } = req.body;

        if (!dataAmanhaParaReserva || !usuarioCreate || !dataReservaExtra) {
            return res.status(422).json({ error: 'Dados inválidos ou ausentes' })
        }

        const salvaConfiguracao = await pool.query("INSERT INTO refeitorio.configuracao (createdate, permissao, dataamanha, dataextra, usuariocreate) VALUES('NOW()', $1, $2, $3, $4)", [reservarSabadoNaSexta, dataAmanhaParaReserva, dataReservaExtra, usuarioCreate])
...
app.get('/buscaConfiguracao', async (req, res) => {
    try {
        const configuracao = await pool.query("SELECT * FROM refeitorio.configuracao ORDER BY id DESC LIMIT 1");
        const dataAmanhaParaReserva = configuracao.rows[0].dataamanha.toISOString().split('T')[0];
        const dataReserva = configuracao.rows[0].dataextra.toISOString().split('T')[0];

        res.status(200).json({ dataamanha: dataAmanhaParaReserva, permissao: configuracao.rows[0].permissao, dataextra: dataReserva });
    } catch (error) {
...
app.post("/salvaReserva", async (req, res) => {
    try {
        ...
        const buscaColaboradorListaFuncionario = await pool.query("SELECT * FROM colaborador.lista_funcionario WHERE matricula = $1", [matricula]);
        if (buscaColaboradorListaFuncionario.rows.length === 0) {
            return res.status(404).json({ error: 'Colaborador não encontrado' });
        }
        const { nome, gerente, nome_setor } = buscaColaboradorListaFuncionario.rows[0];

        const verificaDisponibilidade = await pool.query("SELECT matricula FROM refeitorio.reserva WHERE date_trunc('day', createdate) = date_trunc('day', CURRENT_TIMESTAMP) AND rfid = $1 AND consumido = false ORDER BY id DESC", [codigoRfid]);
        ...
        const dataReserva = nextBusinessDay > dataAmanhaParaReserva ? nextBusinessDay : dataAmanhaParaReserva;
        ...
        const result = await pool.query(
            ...
            [matricula, nome, gerente, nome_setor, opcaoSelecionada.toUpperCase(), codigoRfid, dataReserva]
        );

        const buscaColaboradorRefeitorio = await pool.query("SELECT * FROM colaborador.colaboradores WHERE matricula = $1", [matricula]);
        if (buscaColaboradorRefeitorio.rows.length === 0) {
            await pool.query("INSERT INTO colaborador.colaboradores (rfid, matricula, nome) VALUES ($1, $2, $3)", [codigoRfid, matricula, nome]);
        } else {
            await pool.query("UPDATE colaborador.colaboradores SET rfid= $1, nome = $2 WHERE matricula = $3", [codigoRfid, nome, matricula]);
        }
...
app.get('/buscaReservasDoDia', async (req, res) => {
    try {
        const result = await pool.query("SELECT * FROM refeitorio.reserva WHERE date_trunc('day', data_reserva) = date_trunc('day', CURRENT_TIMESTAMP) ORDER BY id ASC");
...
app.get('/buscaReservasAmanhaDoDia', async (req, res) => {
    try {
        ...
        res.status(500).json({ error: "Erro ao buscar reservas do próximo dia útil" });
    }
});
...
app.post('/entregaReservaPeloRfid', async (req, res) => {
    ...
    try {
        const verificaDisponibilidade = await pool.query("SELECT consumido FROM refeitorio.reserva WHERE date_trunc('day', data_reserva) = date_trunc('day', CURRENT_TIMESTAMP) AND rfid = $1 AND consumido = false ORDER BY id DESC", [codigoRfid]);

        if (verificaDisponibilidade.rowCount > 0) {

            const result = await pool.query("UPDATE refeitorio.reserva SET consumido = true, data_consumo = NOW() WHERE date_trunc('day', data_reserva) = date_trunc('day', CURRENT_TIMESTAMP) AND rfid = $1 RETURNING *", [codigoRfid]);
            if (result.rowCount > 0) {
                return res.status(200).json({ message: "Reserva concluída com sucesso", reserva: result.rows[0] });
            }
...
app.post('/entregaReservaPelaMatricula', async (req, res) => {
    ...
    try {
        const verificaDisponibilidade = await pool.query("SELECT consumido FROM refeitorio.reserva WHERE date_trunc('day', data_reserva) = date_trunc('day', CURRENT_TIMESTAMP) AND matricula = $1 AND consumido = false ORDER BY id DESC", [matricula]);

        if (verificaDisponibilidade.rowCount > 0) {

            const result = await pool.query("UPDATE refeitorio.reserva SET consumido = true, data_consumo = NOW() WHERE date_trunc('day', data_reserva) = date_trunc('day', CURRENT_TIMESTAMP) AND matricula = $1 RETURNING *", [matricula]);
            if (result.rowCount > 0) {
                return res.status(200).json({ message: "Reserva concluída com sucesso", reserva: result.rows[0] });
            }
...
app.post('/salvarReservaSabado', async (req, res) => {
    try {
        const { setorSelecionado, gerenteSelecionado, dataReserva, horaReserva, usuarioCreate, colaboradores, gerenteDestino, setorDestino } = req.body

        if (!setorSelecionado || !gerenteSelecionado || !dataReserva || !horaReserva || !usuarioCreate || !colaboradores || !gerenteDestino || !setorDestino) {
            res.status(422).json({ message: 'Dados obrigatórios não foram preenchidos' })
        } else {
            const salvaReservaSabado = await pool.query("INSERT INTO refeitorio.sabado (createdate, setor_selecionado, gerente_selecionado, data_reserva, hora_reserva, usuariocreate, colaboradores, gerente_destino, setor_destino) VALUES('NOW()', $1, $2, $3, $4, $5, $6, $7, $8);", [setorSelecionado, gerenteSelecionado, dataReserva, horaReserva, usuarioCreate, colaboradores, gerenteDestino, setorDestino])
            res.status(200).json({ message: 'Reserva salva com sucesso' });
        }
    } catch (error) {
...
app.get('/buscaGerenteReservadoSabado', async (req, res) => {
    try {
        ...
        const result = await pool.query("SELECT DISTINCT gerente_destino FROM refeitorio.sabado WHERE data_reserva = $1 ORDER BY gerente_destino ASC", [sabado]);
...
app.get('/geraVoucherPeloGerente', async (req, res) => {
    try {
        ...
        const result = await pool.query("SELECT * FROM refeitorio.sabado WHERE gerente_destino = $1 AND data_reserva = $2", [gerente, dataReserva])
        result.rows.map((item) => {
...
app.get('/geraRelatorioLancheSabado', async (req, res) => {
    try {
        ...
        res.status(500).json({ error: "Erro ao trazer dados do relatório: " + error.message });
    }
});
...
app.get('/geraRelatorioQuantidadeSabado', async (req, res) => {
    try {
        const dataReserva = req.query.dataReserva;
        const relatorio = await pool.query("SELECT DISTINCT data_reserva,  hora_reserva, COUNT(hora_reserva) AS quantidade_hora FROM refeitorio.sabado WHERE data_reserva = $1 GROUP BY data_reserva, hora_reserva", [dataReserva])
...
app.get("/buscaDadoGlobaisAuditorias", async (req, res) => {
    try {
        const result = await pool.query("SELECT EXTRACT(MONTH FROM data_reserva) AS mes, EXTRACT(YEAR FROM data_reserva) AS ano, COUNT(*) AS total_reservado, COUNT(CASE WHEN consumido THEN 1 END) AS total_consumido FROM refeitorio.reserva WHERE data_reserva < CURRENT_DATE AND EXTRACT(YEAR FROM data_reserva) = EXTRACT(YEAR FROM CURRENT_DATE) GROUP BY EXTRACT(YEAR FROM data_reserva), EXTRACT(MONTH FROM data_reserva) ORDER BY ano, mes;");
